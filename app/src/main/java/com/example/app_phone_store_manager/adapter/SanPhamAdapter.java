...
import com.example.app_phone_store_manager.dao.DaoHang;
import com.example.app_phone_store_manager.dao.DaoThuocTinhSanPham;
...
import com.example.app_phone_store_manager.model.ThuocTinhSanPham;
import com.example.app_phone_store_manager.utilities.ItemSanPhamClick;
    ...
    public Filter getFilter() {
        return new Filter() {
            ...
            protected FilterResults performFiltering(CharSequence constraint) {
                String serSP = constraint.toString();
                if (serSP.isEmpty()) {
                    list = mListSPold;
                } else {
                    List<SanPham> listSP = new ArrayList<>();
                    for (SanPham sanPham : mListSPold) {
                        if (sanPham.getMaSP().toLowerCase().contains(serSP.toLowerCase()) ||
                                sanPham.getTenSP().toLowerCase().contains(serSP.toLowerCase())) {
                            listSP.add(sanPham);
                        }
                    }
                    list = listSP;
